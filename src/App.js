...
class App extends Component {
  ...
  async fetch() {
    ...
    let enhancedResponse = await fetch(ENHANCED_API, {
      body: JSON.stringify({
        flow: [
          {
    ...
    let addrData = {};
    enhancedData.items.map(item => {
      addrData[item.identity] = item;
    })
    ...
    return (
      <div className="App">
        <header className="App-header">
          <h1 className="App-title">Welcome to Governance Enhanced</h1>
        ...
        <ul>
          {Object.keys(this.state.age).map(answer => <li key={answer}>{answer}: {parseInt(this.state.age[answer])} days</li>)}
        </ul>
